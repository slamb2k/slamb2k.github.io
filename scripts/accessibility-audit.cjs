...
const WCAG_AA_CRITERIA = {
  ...
  '2.5.4': {
    ...
    level: 'A',
    description: 'Functionality triggered by device motion can be disabled and has alternative input',
    automated: false
  },
  ...
  '3.3.4': {
    ...
    level: 'AA',
    description: 'For pages with legal, financial, or data modification, submissions are reversible, checked, or confirmed',
    automated: false
  },
  ...
  '4.1.2': {
    ...
    level: 'A',
    description: 'UI components have names, roles, and values that can be programmatically determined',
    automated: true
  },
  '4.1.3': {
...
function generateAuditReport() {
  ...
  const automatedCriteria = Object.entries(WCAG_AA_CRITERIA).filter(([, criteria]) => criteria.automated);
  ...
  const manualCriteria = Object.entries(WCAG_AA_CRITERIA).filter(([, criteria]) => !criteria.automated);
  ...
  // Compliance Score
  const automatedPassing = automatedCriteria.filter(([number]) => getAutomatedTestStatus(number) === 'pass').length;
...
function getAutomatedTestStatus(criteriaNumber) {
  ...
  const implemented = [
    '1.1.1', '1.3.1', '1.3.2', '1.3.5', '1.4.3', '1.4.11',
    '2.1.1', '2.1.2', '2.4.1', '2.4.2', '2.4.3', '2.4.4',
    '2.4.6', '2.4.7', '2.5.3', '3.1.1', '3.1.2', '3.3.1',
