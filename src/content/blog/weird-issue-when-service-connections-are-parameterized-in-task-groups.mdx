...
I was showing someone my blog the other day and they asked when my last post was. I confidently said, "About a month or so ago.." and then I saw it... !(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894649956.png) Where did all those months go? Maybe I blacked out out or something... In order to ease into my "blogging resurgence", I thought I would start with a quick one regarding how I use VSTS and an interesting issue I encountered last night. Since this problem owes me a few hours, I hope it shortcuts somebody else's pain.  I love how VSTS gives teams the flexibility to work in whatever way they like but a framework still exists to put some guard rails in place. More important than governance though is the ability to share proven DevOps patterns so team members can focus on the unique aspects of the solution rather than common activities like deployment. **Scenario**

1.  A series of environments in a (https://en.wikipedia.org/wiki/Development,_testing,_acceptance_and_production) (Dev, Testing, Acceptance, Prod) pipeline.
2.  Each environment is a single web app that we wish to run as PaaS.
3.  We would like to use deployment slots to perform a (https://martinfowler.com/bliki/BlueGreenDeployment.html) when releasing a build.
...
3.  Add the required tasks for a blue/green deployment to the environment . i.e. (https://en.wikipedia.org/wiki/Development,_testing,_acceptance_and_production), (https://docs.microsoft.com/en-us/vsts/pipelines/tasks/test/cloud-based-web-performance-test?view=vsts), (https://docs.microsoft.com/en-us/vsts/pipelines/tasks/deploy/azure-app-service-manage?view=vsts).

!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894650321.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894650468.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894650848.png)

The only property that doesn't love this strategy is the _**Azure Subscription**_. In a normal build or release it is not possible to use a variable to specify the Azure Subscription normally but at some stage the VSTS team added the ability to use a (https://docs.microsoft.com/en-us/vsts/pipelines/tasks/deploy/azure-app-service-manage?view=vsts). The issue I discovered last night is that there is some sensitivity however when it comes to the name of this variable. _**IMPORTANT!!! If you try to use a variable name for the Azure Subscription with a hyphen in it like $(Subscription-Name), VSTS will attempt to verify the subscription and report a validation error. This will prevent you from saving the task group.**_

!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894650997.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894651327.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894651638.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894651960.png)
...
!(/images/blog/weird-issue-when-service-connections-are-parameterized-in-task-groups-1758894652261.png)
